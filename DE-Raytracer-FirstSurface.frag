...
// 1 = normal, find 1st surface, 2 = find 2nd surface
uniform int SurfaceIndex;  slider[1,1,2]

// hop far to advacne before 2nd surface search
uniform float SurfaceGap; slider[1,1,2]
...
vec3 trace(vec3 from, vec3 dir, inout vec3 hit, inout vec3 hitNormal) {
 ...
 // ====================================================================
	for (steps=0; steps<MaxRaySteps; steps++) {
		...
		if (dist < epsModified) {		// hit surface
			// move back
    ...
    hitSurface = true;
		...
		if (totalDist > MaxDistance) { // missed surface
			fSteps -= (totalDist-MaxDistance)/dist;
			break;
		}
	}
 // ====================================================================
  if(hitSurface && SurfaceIndex == 2) {
    float d1 = dist;
	   float td1 = totalDist;
    vec4 ot1 = orbitTrap;
			hitSurface = false;

    totalDist *= SurfaceGap;
	  ...
	  if(!hitSurface) {  // reert to previous surface
		 		dist = d1;
				totalDist = td1;
				orbitTrap = ot1;
	 		}
